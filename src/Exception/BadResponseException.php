...
class BadResponseException extends ResponseException
{
    ...
    public function __construct(Response $response, ?Throwable $previous = null)
    {
        ...
        $statusMessage = self::STATUS_TEXTS[$statusCode] ?? 'Unknown';
        $message = $response->getErrorMessage() ?: $response->getData();
        if (! is_string($message)) {
            $message = json_encode($message, JSON_THROW_ON_ERROR);
        }

        $message = sprintf('Response not OK [%u %s]: %s', $response->getStatus(), $statusMessage, $message);
